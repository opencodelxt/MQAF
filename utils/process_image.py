...
class RandCrop_LIVE(object):
    ...
    def __call__(self, sample):
        ...
        c, h_ref, w_ref = r_img.shape
        c, h_dis, w_dis = d_img.shape
        ...
        for _ in range(self.num_crop):
            top = np.random.randint(0, h_ref - new_h)
            left = np.random.randint(0, w_ref - new_w)
            tmp_r_img = r_img[:, top: top + new_h, left: left + new_w]
            ret_r_img += tmp_r_img

        for _ in range(self.num_crop):
            top = np.random.randint(0, h_dis - new_h)
            left = np.random.randint(0, w_dis - new_w)
...
def five_point_crop_live(idx, img, config):
    ...
    if idx == 0:
        ...
        left = center_w - new_w // 2
    if len(img.shape) == 3:
        img_org = img[: , top: top + new_h, left: left + new_w]
    else:
        img_org = img[ :,: , top: top + new_h, left: left + new_w]
    return img_org
